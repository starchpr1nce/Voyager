...
final class Stage4Game1ViewModel: ObservableObject {
    @Published var state: Stage4GameState = .text0
    @Published var textOutput = ""
    @Published var phraseSource = Stage4LotteryExpensiveReference.shared.phrases[0]
    ...
    func nextText(_ state: Stage4GameState) {
        if index < Stage4LotteryExpensiveReference.shared.phrases.count - 1 {
            self.state = state
            index += 1
            textOutput = ""
            phraseSource = Stage4LotteryExpensiveReference.shared.phrases[index]
        } else {
            textOutput = ""
            self.state = state
        }
